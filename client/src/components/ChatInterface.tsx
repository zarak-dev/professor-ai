  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] max-w-4xl mx-auto px-2 sm:px-4 py-4">
      <div className="border border-slate-200 bg-white rounded-xl shadow-xs flex flex-col flex-1 overflow-hidden">
        ...
        >
          {initialLoading ? (
            ...
            messages.map((msg) => (
              ...
              >
                {msg.sender === 'ai' && (
                  <div className="w-7 h-7 rounded-lg bg-[#F0F5F8] text-[#213448] flex items-center justify-center shrink-0 border border-[#CADDE6] mt-0.5">
                    <Bot size={14} className="text-[#547792]" />
                  </div>
                )}
                <div
                  className={`rounded-xl px-4 py-3 text-sm leading-relaxed max-w-[85%] sm:max-w-[78%] ${
                    msg.sender === 'user'
                      ? 'bg-[#213448] text-white shadow-xs'
                      : 'bg-white text-[#213448] border border-[#E2DBD0] shadow-xs'
                  }`}
                >
                  {msg.sender === 'user' ? (
                    <p className="whitespace-pre-wrap">{msg.text}</p>
                  ) : (
                    <div className="prose prose-sm max-w-none text-[#213448]">
                      <ReactMarkdown
                        components={{
                          ...
                          strong: ({ children }) => <strong className="font-semibold text-[#213448]">{children}</strong>,
                          code: ({ children }) => (
                            <code className="bg-[#F4EFE6] text-[#213448] px-1.5 py-0.5 rounded font-mono text-xs border border-[#E2DBD0]">
                              {children}
                            </code>
                          ),
                          a: ({ href, children }) => (
                            <a href={href} className="text-[#547792] underline font-medium hover:text-[#213448]">
                              {children}
                            </a>
                          ),
                        }}
                  ...
                  <span
                    className={`block text-[10px] mt-1 select-none ${
                      msg.sender === 'user' ? 'text-white/70 text-right' : 'text-[#94B4C1]'
                    }`}
                  >
          ...
          {loading && (
            ...
            >
              ...
              <div className="bg-white text-[#213448] rounded-xl px-4 py-3 border border-[#E2DBD0] shadow-xs flex items-center gap-2">
                <Loader2 size={15} className="animate-spin text-[#547792]" />
                <span className="text-xs font-medium text-[#547792]">The Professor is thinking...</span>
              </div>
            </motion.div>
          )}
        </div>
        ...
        {/* Input Composer */}
        <div className="p-3 sm:p-4 border-t border-[#E2DBD0] bg-white">
          <form onSubmit={sendMessage} className="flex items-end gap-2">
            <textarea
              ...
              className="flex-1 bg-white border border-[#E2DBD0] rounded-lg px-3.5 py-2.5 text-sm text-[#213448] placeholder:text-[#94B4C1] focus:outline-none focus:ring-2 focus:ring-[#547792] focus:border-transparent resize-none min-h-[44px] max-h-[120px] transition-all"
              rows={1}
              disabled={loading}
          ...
          <div className="hidden sm:flex items-center justify-between text-[11px] text-[#94B4C1] mt-2 px-1">
            <span>Press <kbd className="px-1 py-0.5 bg-[#F4EFE6] border border-[#E2DBD0] rounded text-[10px] text-[#213448]">Enter</kbd> to send, <kbd className="px-1 py-0.5 bg-[#F4EFE6] border border-[#E2DBD0] rounded text-[10px] text-[#213448]">Shift + Enter</kbd> for newline</span>
            <span className="flex items-center gap-1">
              <Sparkles size={11} className="text-[#547792]" /> Grounded in document text
            </span>
          </div>
        </div>
      </div>
